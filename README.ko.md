...
`mode: 'horizontal'`과 `rotation.origin: 'bottom-center'`를 함께 쓰면 아래쪽 축을 기준으로 좌우로만 움직이는 느낌을 만들 수 있습니다.
...
`rotation.origin`은 swipe 인식이나 dismiss 속도가 아니라 회전 기준점을 제어합니다.
...
같은 `maxDegrees` 값도 `bottom-center`에서는 더 강하게 느껴집니다.
그래서 Tinder preset은 `maxDegrees`를 직접 지정하지 않은 경우 `bottom-center`에 더 작은 기본 회전 각도를 사용합니다.
...
- 기본값 `1.5`는 카드를 `deckWidth * 1.5` 위치로 보냅니다.
