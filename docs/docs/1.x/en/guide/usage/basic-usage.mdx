...
- Omit `allowedDirections` to allow left and right.
- Include `'up'` explicitly to allow upward dismisses.
- Gesture up requires `SwipeDeckMotion.tinder({ drag: { mode: 'free' } })`, the default.
- `drag.mode: 'horizontal'` rejects gesture up but does not block `actions.swipeUp()`.
